...
import type { DashboardSummary, UpdateBookCatalogPayload } from './interfaces/admin.interface';
...
export class AdminService {
  ...
  async getDashboardSummary() {
    try {
      ...
      return { success: true, message: SUCCESS_MESSAGES.DASHBOARD_FETCHED, data };
    } catch (error) {
  ...
  async getSellers(page = 1, limit = 10, status?: string) {
    try {
      ...
      return { success: true, message: SUCCESS_MESSAGES.SELLER_FETCHED, data, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Unknown';
  ...
  async getBooks(page = 1, limit = 10, status?: string) {
    try {
      ...
      return { success: true, message: SUCCESS_MESSAGES.BOOKS_FETCHED, data, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Unknown';
  ...
  async updateBookCatalog(bookId: string, payload: UpdateBookCatalogPayload) {
    try {
      const book = await this.bookModel.findByIdAndUpdate(bookId, payload, { new: true }).exec();
      if (!book) throw new NotFoundException(ERROR_MESSAGES.BOOK_NOT_FOUND);
  ...
  async getCustomers(page = 1, limit = 10, status?: string) {
    try {
      ...
      return { success: true, message: SUCCESS_MESSAGES.CUSTOMER_FETCHED, data, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Unknown';
